<script lang="ts">
  ...
  interface Props {
    ...
    onOpenInNewTab: () => void;
    /** Optional callback for "Properties" action */
    onProperties?: () => void;
    /** Callback to dismiss the menu */
    onClose: () => void;
  }

  let { x, y, visible, onOpen, onOpenInNewTab, onProperties, onClose }: Props = $props();
  ...
  let adjustedY = $derived(Math.min(y, (browser ? window.innerHeight : 9999) - 140));
</script>
...
{#if visible}
  ...
  >
    ...
    {#if onProperties}
      <button class="context-menu-item" role="menuitem" onclick={handleProperties}>
        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <circle cx="12" cy="12" r="3"></circle>
