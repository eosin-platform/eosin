...
class VAEConfig:
    ...
    # Model architecture (~8M params with these defaults)
    base_channels: int = 32
    channel_multipliers: Tuple[int, ...] = (1, 2, 4, 8)
    latent_channels: int = 4
    downsample_factor: int = 16  # Total downsampling (256 / 16 = 16x16 latent)
    num_res_blocks_per_stage: int = 2
    use_attention_at: Tuple[int, ...] = (16,)  # Spatial sizes where attention is applied
    ...
    kl_warmup_steps: int = 10000  # Steps to anneal KL weight from 0 to beta
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        for i, mult in enumerate(channel_multipliers):
            ...
            stage["attn"] = None  # Placeholder, actual attention created if needed
...
class Decoder(nn.Module):
    ...
    def forward(self, z: torch.Tensor, target_size: Optional[int] = None) -> torch.Tensor:
        ...
        # Sigmoid to ensure output is in [0, 1] range for images
...
class VAE(nn.Module):
    ...
    def reparameterize(
        ...
        Args:
            mu: Mean of latent distribution
            logvar: Log variance of latent distribution
            training: If False, returns mu directly (deterministic)
...
def kl_divergence(mu: torch.Tensor, logvar: torch.Tensor) -> torch.Tensor:
    ...
    Returns mean KL per batch element (scalar).
    
    Args:
        mu: Mean of latent distribution, shape (B, C, H, W)
        logvar: Log variance, shape (B, C, H, W)
    ...
    # This gives us the total KL per sample, then average over batch
    ...
    def _extract_random_tile(self, max_attempts: int = 50) -> Tuple[Optional[Image.Image], str]:
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                arr = np.array(img)
                near_black_mask = (arr[:, :, 0] < 4) & (arr[:, :, 1] < 4) & (arr[:, :, 2] < 4)
    ...
    def __getitem__(self, idx: int) -> torch.Tensor:
        ...
        while total_attempts < max_total_attempts:
            ...
            if total_attempts % 100 == 0:
                print(f"Warning: Struggled to find valid tile after {total_attempts} attempts. {debug_info}")
...
def main():
    ...
    # Create KL scheduler
    kl_scheduler = LinearKLScheduler(
        beta=config.beta,
